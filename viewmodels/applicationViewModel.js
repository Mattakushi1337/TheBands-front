...
class ApplicationViewModel {
    ...
    async approveApplication(applicationId, role) {
        const response = await axios.patch(`http://192.168.1.103:3000/applications/${applicationId}/approve`, { role });
        return response.data;
    };

    async rejectApplication(applicationId, role) {
        const response = await axios.patch(`http://192.168.1.103:3000/applications/${applicationId}/reject`, { role });
        return response.data;
    };

    async createApplication(bandId, role) {
        console.log("model", bandId);
        const response = await axios.post(`http://192.168.1.103:3000/applications/${bandId}/join`, { role });
        return response.data;
    };
}
